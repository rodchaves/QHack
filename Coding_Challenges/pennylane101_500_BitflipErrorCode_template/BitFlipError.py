# -*- coding: utf-8 -*-
"""
Created on Tue Feb 15 21:21:06 2022
...
def error_wire(circuit_output):
    ...
    probs = list(circuit_output)
    max_index = probs.index(max(probs))
    new_probs = probs
    while max_index == 0 or max_index == 4:
        new_probs[max_index]=0
        max_index = probs.index(max(new_probs))
        
    if probs[max_index]==0:
        return [1,0,0,0]
    elif max_index == 1 or max_index == 5:
        return [1-probs[1]-probs[5],0,0,probs[1]+probs[5]]
    elif max_index == 2 or max_index == 6:
        return [1-probs[2]-probs[6],0,probs[2]+probs[6],0]
    else:
        return[1-probs[3]-probs[7],probs[3]+probs[7],0,0]
