# -*- coding: utf-8 -*-
"""
Created on Tue Feb 15 20:54:50 2022
...
def my_finite_diff_grad(params):
    ...
    for i in range(len(params)):
        # QHACK # 
            delta = 0.00001
            parameters = params
            pshift = params[i]+delta/2
            mshift = params[i]-delta/2
            parameters[i] = pshift 
            first_term = cost(parameters)
            parameters[i] = mshift
            second_term = cost(parameters)
            gradients[i]=(first_term-second_term)/delta
        # QHACK #
