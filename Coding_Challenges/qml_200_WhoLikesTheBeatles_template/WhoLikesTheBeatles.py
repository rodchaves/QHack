# -*- coding: utf-8 -*-
"""
Created on Sat Feb 19 14:08:25 2022
...
def distance(A, B):
    ...
    n_qubits = int(np.ceil(np.log2(len(A))))
    dev = qml.device("default.qubit", wires = 3*n_qubits)
    @qml.qnode(dev)
    def SWAPtest(inp):
        qml.AmplitudeEmbedding(features = inp, wires = range(1,3*n_qubits), normalize = True)
    ...
    inp = np.kron(A,B)
    angle = np.sqrt(2*SWAPtest(inp)[0]-1)
    dist = np.sqrt(2*(1-angle))
    return dist
    # QHACK #
